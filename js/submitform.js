$(document).ready(function(){
	$("#submit").click(function(){					   				   
		...
		if(emailToVal == '') {
			$("#emailTo").after('<span class="error">I think You forgot to enter your Email id</span>');
			hasError = true;
		...
		if(emailFromVal == '') {
			$("#emailFrom").after('<span class="error">I think You forgot to enter your Email id</span>');
			hasError = true;
		...
		if(messageVal == '') {
			$("#message").after('<span class="error">Hey, do you not have anything to say?</span>');
			hasError = true;
		}
   				...
   				{ emailTo: emailToVal, emailFrom: emailFromVal, subject: subjectVal, message: messageVal },
   					function(data){
						$("#sendEmail").slideUp("normal", function() {				   
							...
							$("#sendEmail").before('<h1>Success</h1><p>Your message has been sent succesfully. Thank you ! ! !</p>');											
						});
   					}
				 );
		}
